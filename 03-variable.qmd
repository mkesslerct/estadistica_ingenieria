...
$F_X(t)=\mathbb{P}(X\leq t)=\mathbb{P}[(X=0)\cup (X=1)]=%
\mathbb{P}[X=0]+\mathbb{P}[X=1]=1/8+3/8=1/2$. Si $2\leq t<3$, el suceso $(X\leq t)$ se
cumple si y solamente si $X$ toma el valor 0, 1 ó 2, es decir
$F_X(t)=\mathbb{P}(X\leq  t)=\mathbb{P}[X=0]+\mathbb{P}%
[X=1]+\mathbb{P}[X=2]=1/2+3/8=7/8$. Finalmente, si $t>3$, el suceso $(X\leq t)$
es el suceso seguro puesto que todos los valores que puede tomar $X$ son
...
numerable [^1] de valores, decimos que $X$ es una variable discreta
:::
...
\begin{align}
    \bullet& 0\leq f(x)\quad \mbox{para }x=x_1,\ldots,x_n,\ldots\\
    \bullet& \sum_{x_i}f_X(x_i)=1.
\end{align}
...
Por la definición @def-esperanzaf, deducimos que $\mathop{var}(X)$ se puede calcular como
...
**Fórmula equivalente para el cálculo de la varianza.** Tenemos
...
alrededor de su media. En particular, si la desviación típica de $X$ es
...
-   Distribución - Función puntual de probabilidad.
    ...
    convención $0!=1$.\
    Nota: ¿se cumple que $\sum_{i=1}^n ( \begin{array}{c} n\\ i 
      \end{array})p^i(1-p)^{n-i}=1$? La respuesta es sí, por el binomio
...
función de densidad de la v.a $X$. Notar que se trata de una
...
La función de distribución acumulada de $X$ es, ver sección
...
1.  $f(x)\geq 0,\quad\mbox{para todo $x\in \mathbb{R}$},$
...
interpreta como el centro de gravedad de la distribución, ver figura
@fig-esperanza.
...
duración de los dispositivos electrónicos de la sección
...
la figura @fig-uniforme. Una v.a $X$ que tenga esta función de
densidad se llama una v.a uniforme entre $a$ y $b$. Lo denotaremos por
$$X\sim {\mathcal U}([a,b]).$$

::: center
![ Densidad de una v.a uniforme](figures/densidadunif.png){#fig-uniforme}
:::
...
-   La función de densidad de una v.a $X\sim {\mathcal Exp}(\lambda)$ es
    ...
    sección @sec-ejemplo_dura. De hecho, resulta que la densidad de
...
-   *Esperanza y varianza*. Demostramos de la misma manera que para el
    ...
    @sec-ejemplo_dura, utilizando la integración por partes
...
-   *Propiedad de falta de memoria de la distribución exponencial.* La
    ...
    $t_1>0,\ t_2>0$, $$\mathbb{P}(X>t_1+t_2| X> t_1)=\mathbb{P}(X>t_2).$$ Por la
...
cuya representación gráfica es la famosa "campana de Gauss", ver Figura
@fig-densnorm.

::: center
![ Densidad Normal](figures/densidadnorm.png){#fig-densnorm width="7cm"}
:::
...
sección @sec-ejempl-newcomb, podemos comprobar que las frecuencias
de aparición de los datos experimentales se ajustan bastante bien a un
modelo Normal. En la figura @fig-newcombfit, se ha ajustado una curva Normal al
...
::: center
![ Ajuste de una densidad Normal al histograma de
Newcomb](figures/newcombfit.png){#fig-newcombfit}
:::
...
-   *Esperanza y varianza:* Es posible comprobar que, si
...
-   *Función de distribución acumulada.* La función $f_X$ no admite
    ...
    de datos como R disponen de algoritmos que permitan calcular para
...
\end{align}
 ...
 lo que queda reflejado en la figura
    ...
    ::: center
    ![ Regla del 68% - 95% - 99.7%](figures/normal.png){#fig-normal}
    :::
...
:   Para una distribución $Z\sim{\mathcal N}(0,1)$.\
    ...
    que sabeis calcular las probabilidades siguientes:\
...
:   Para una distribución $X\sim{\mathcal N}(\mu,\sigma^2)$.\
    ...
    $Z=\frac {X-\mu}{\sigma}\sim{\mathcal N}(0,1)$ se llama tipificar la
...
para la v.a Normal $W$, tal como está ilustrado en la Figura
@fig-binnormal,
para unos pocos valores de $X$.

::: center
![ Aproximación de una distribución Binomial por una distribución
Normal](figures/binnormal.png){#fig-binnormal}
:::
...
[^1]: Un conjunto infinito numerable es un conjunto del que se puede
