...
samples = 1 + np.concatenate(
    [np.arange(m).reshape(-1, 1), np.arange(m).reshape(-1, 1)],
    axis=1
)
means = rng.normal(mu, sigma/np.sqrt(n), size=(m, 1)) 
ends = (
    means +
    np.array(
        [-1.96 * sigma / np.sqrt(n), 1.96 * sigma / np.sqrt(n)]
    ).reshape(1,-1)  
)
segs = np.zeros((m, 2, 2))
segs[:, :, 1] = ends
